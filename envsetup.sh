...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^kanged_") ; then
        KANGED_BUILD=$(echo -n $1 | sed -e 's/^kanged_//g')
    else
        KANGED_BUILD=
    fi
    export KANGED_BUILD
...
function lunch()
{
    ...
    then
        # if we can't find a product, try to grab it off the ProjectKanged-devices GitHub
        T=$(gettop)
        cd $T > /dev/null
        vendor/kanged/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/kanged/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $KANGED_FIXUP_COMMON_OUT ]; then
...
function embark() {
    ...
    if [ -z "$device" ]; then
        echo "Correct usage: embark <device_codename> [build_type]"
    ...
    case "$build_type" in
        user|userdebug|eng)
        lunch kanged_"$device"-"$build_type"
        ;;
        *)
